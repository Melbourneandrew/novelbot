...
export const GET = ProtectedRoute(
  ...
  async (request: AuthenticatedNextRequest) => {
    ...
    if (!activeSubscription) {
      return new NextResponse("No active subscription", {
        status: 400,
      });
    }
    if (!stripeSubscriptionObject) {
      return new NextResponse(
        "Can't find active subscription in stripe",
        { status: 400 }
      );
    }
    if (!activeSubscription.stripeSubscriptionId) {
      return new NextResponse("No active subscription id", {
        status: 400,
      });
    }
    ...
    if (!subscriptionCancelled) {
      return new NextResponse("Can't cancel subscription", {
        status: 400,
      });
    }

    return new NextResponse("Subscription cancelled", {
      status: 200,
    });
  }
);
