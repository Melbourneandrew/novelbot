...
export async function POST(request: NextRequest) {
    ...
    const user = await UserService.requestPasswordReset(email, passwordResetToken)
    if (!user) {
        return new Response("User not found", { status: 404 });
    }

    const emailSent = await sendEmail(email, "Password Reset", `Reset your password here: ${APP_URL}/protected/reset-password?token=${passwordResetToken}`);
    if (!emailSent) {
