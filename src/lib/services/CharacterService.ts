...
export async function findCharactersByAuthor(
  ...
  const books = await AuthorService.findBooksByAuthor(authorId);
  if (books == []) {
    return [];
  }
...
export async function findCharactersWithStatsByAuthor(
  ...
  const characterStats = characters.map((character) => {
    const characterConversations = conversations.filter(
      (conversation) =>
        conversation.character.toString() === character._id.toString()
    );
    const totalConversations = characterConversations.length;
    const totalMessages = characterConversations.reduce((acc, conversation) => {
      return acc + conversation.messages.length;
    }, 0);
    const averageConversationLength = totalMessages / totalConversations;
    return {
      ...character.toObject(),
      totalConversations,
